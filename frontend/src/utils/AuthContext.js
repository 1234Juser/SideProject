...
export const AuthProvider = ({ children }) => {
    ...
    useEffect(() => {
        ...
        try {
            ...
            setIsAuthInitialized(true); // 인증 정보 로딩 완료
        }
    }, []);


    // 로그인 처리 함수
    const login = (accessToken, memberId, memberUsername, memberRole, memberNickname) => {
        try {
    ...
    const logout = () => {
        try {
            ...
            setAuth({ isAuthenticated: false, memberId: null,  accessToken: null, memberUsername: null, memberRole: null, memberNickname: null });
